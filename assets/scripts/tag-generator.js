(function($) {
    ...
    au_cf7_controllable_fields.taggen.updateOption = function(e) {
        var $this = $(e.currentTarget),
            ...
            $option = $this.prev('input[type="hidden"].option');
        $option.val(value);
        if (e.type != 'change') {
            // CF only listens for "change" so force the tag to rebuild for our other listeners
    ...
    $(function() {
        $('form.tag-generator-panel input.au-cf-option').on('change keyup click', au_cf7_controllable_fields.taggen.updateOption);
        ...
        $('.contact-form-editor-panel #tag-generator-list a.thickbox.button[href*="inlineId=tag-generator-panel-"][href*="control"]').addClass('controller-form-tag');
    });
})(jQuery);