 ...
 * Get Minified
 *
 * Assets are minified on production and staging environments where WP_DEBUG is disabled and not refreshing cache.
 *
 * @return string If assets should be minified, the string will return `.min`. Otherwise, it will be empty.
 */
function au_cf7_cf_get_minified()
{
    $is_prod = in_array(wp_get_environment_type(), array('production', 'staging'));
    $is_debug = defined('WP_DEBUG') || constant('WP_DEBUG');
    $is_cache = array_key_exists('nocache', $_REQUEST) || array_key_exists('preview', $_GET) || (array_key_exists('avoid-minify', $_GET) && $_GET['avoid-minify'] == 'true') || isset($wp_customize);
    return  $is_prod && !$is_debug && !$is_cache ? '.min' : '';
}
